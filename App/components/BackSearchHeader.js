...
export default function BackSearchHeader({ navigation }) {
    const [searchText, setSearchText] = useState('');

    return (
        <View style={{ width: '100%', height: AdjustFontSize(70), backgroundColor: '#DAC7ED', justifyContent: 'center', flexDirection: 'row' }}>
            <TouchableOpacity onPress={() => navigation.goBack()} style={{ flex: .14, justifyContent: 'center', }}>
                <IconBack name='chevron-back' size={AdjustFontSize(30)} color='#fff' style={{ alignSelf: 'center' }} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => navigation.openDrawer()} style={{ flex: .14, justifyContent: 'center', }}>
                <IconMaterial name='menu' size={AdjustFontSize(30)} color='#fff' />
            </TouchableOpacity>
            <TouchableOpacity
